...
const MedicalSchedules: React.FC = () => {
  ...
  const { data: meetingSchedulesData, refetch: refetchMeetingSchedule } =
    useQuery({
  ...
  const events: CalendarEvent[] =
    meetingSchedulesData &&
    meetingSchedulesData.data &&
    Array.isArray(meetingSchedulesData.data.data) &&
    meetingSchedulesData.data.data.length > 0
      ? (meetingSchedulesData.data.data
          .map((meeting: any) => {
            try {
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {daysInMonth.map((date, index) => {
          ...
          const remainingEvents =
            dayEvents.length > 4 ? dayEvents.length - 4 : 0;
                  ...
                  <div className="mt-1 space-y-1">
                    {dayEvents.slice(0, 4).map((event) => (
                      <div
                        ...
                        title={`${event.title} (${
                          event.type === "PAID" ? "Gói có phí" : "Gói miễn phí"
                        })`}
                      >
                        {event.title}
                      </div>
  ...
  return (
    <div className="container mx-auto p-4">
      <CalendarHeader
        handleCreate={() =>
          setDialogStatus((prev) => ({ ...prev, isCreate: true }))
        }
      ...
      />
      {renderView()}
      <CreateMedicalScheduleWithoutOrderAdminDialog
        onCreateSuccess={() => {
          refetchMeetingSchedule();
        }}
        open={dialogStatus.isCreate}
        onOpenChange={() =>
          setDialogStatus((prev) => ({ ...prev, isCreate: false }))
        }
      />
    </div>
  );
