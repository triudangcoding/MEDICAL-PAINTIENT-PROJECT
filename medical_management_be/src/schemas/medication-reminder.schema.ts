...
export const quickConfirmSchema = z.object({
  prescriptionItemId: z.string().min(1, 'ID chi tiết thuốc không được để trống').refine(
    (val) => {
      // Cho phép cả UUID và format khác như "item-123-2024-01-15-08:00"
      return val.length > 0;
    },
    { message: 'ID chi tiết thuốc không hợp lệ' }
  ),
