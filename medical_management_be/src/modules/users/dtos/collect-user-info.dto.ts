...
const CollectUserInfoSchema = z
  .object({
    ...
    educationLevel: z.enum(
      ['CAP_1', 'CAP_2', 'CAP_3', 'DAI_HOC_CAO_DANG', 'SAU_DAI_HOC'],
      {
    ...
    diagnosisMonthsAgo: z
      .number()
      .int()
      .min(0, { message: 'Thời gian chẩn đoán không hợp lệ' }),
    cancerStage: z.enum(
      ['GIAI_DOAN_1', 'GIAI_DOAN_2', 'GIAI_DOAN_3', 'GIAI_DOAN_4', 'TAI_PHAT'],
      {
    ...
    currentTreatmentMethod: z.enum(
      ['PHAU_THUAT', 'XA_TRI', 'HOA_TRI', 'THUOC_DICH', 'THUOC_MIEN_DICH'],
      {
        errorMap: () => ({
          message: 'Phương pháp điều trị hiện tại không hợp lệ'
        })
      }
    ),
    ...
    hospitalInfo: z
      .string()
      .min(1, { message: 'Thông tin bệnh viện không được để trống' }),
    height: z
      .number()
      .int()
      .min(50, { message: 'Chiều cao không hợp lệ' })
      .max(250, { message: 'Chiều cao không hợp lệ' }),
    weight: z
      .number()
      .int()
      .min(20, { message: 'Cân nặng không hợp lệ' })
      .max(200, { message: 'Cân nặng không hợp lệ' }),
    ...
    hasPsychologicalIssues: z
      .enum(['CO', 'KHONG'], {
        errorMap: () => ({ message: 'Trạng thái vấn đề tâm lý không hợp lệ' })
      })
      .optional(),
    hasNutritionalIssues: z
      .enum(['CO', 'KHONG'], {
        errorMap: () => ({
          message: 'Trạng thái vấn đề dinh dưỡng không hợp lệ'
        })
      })
      .optional()
  })
