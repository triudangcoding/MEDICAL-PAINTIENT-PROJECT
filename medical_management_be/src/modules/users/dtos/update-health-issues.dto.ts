...
const UpdateExerciseVideosSchema = z
  .object({
    ...
    exerciseVideoIds: z
      .array(
        ...
        {
          errorMap: () => ({
            message: 'Danh sách mã video bài tập không hợp lệ'
          })
        }
      )
      .optional(),
  ...
  .refine(
    (data) => {
      ...
      return (
        data.exerciseVideoIds !== undefined ||
    ...
    {
      message:
        'Cần cung cấp ít nhất một thông tin để cập nhật (exerciseVideoIds, hasPsychologicalIssues, hoặc hasNutritionalIssues)',
      path: ['root']
    }
  );

class UpdateHealthAndExerciseDto extends createZodDto(
  UpdateExerciseVideosSchema
) {}
